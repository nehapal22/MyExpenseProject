...
def forecast(request):
    ...
    data = pd.DataFrame({
        ...
        'Category': [getattr(expense, 'category', 'Unknown') for expense in expenses]
    })
    data.set_index('Date', inplace=True)
    
    # Group the data by Category and calculate the total expenses per category
    category_forecasts = data.groupby('Category')['Expenses'].sum().to_dict()

    # Fit ARIMA model to the expenses data
    model = ARIMA(data['Expenses'], order=(5, 1, 0))
    model_fit = model.fit()

    # Forecast the next 30 days of expenses starting from the next day
    forecast_steps = 30
